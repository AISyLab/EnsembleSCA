...
def aes_labelize(trace_data, leakage_model, byte, param, key=None):
    ...
    if key is not None:
        ...
        if param["random_key"]:
            key_byte = [row[byte + param["key_offset"]] for row in trace_data]
...
def open_datasets(dataset, n_train, n_val, n_test):
    hf = h5py.File('../datasets/' + dataset + '.h5', 'r')
    ...
    z_score_mean, z_score_std = create_z_score_norm(train_samples)
    apply_z_score_norm(train_samples, z_score_mean, z_score_std)
    apply_z_score_norm(test_samples, z_score_mean, z_score_std)

    t_data = train_data[0:n_train]
    v_data = train_data[n_train:n_train + n_val]
    v_samples = train_samples[n_train:n_train + n_val]
    t_samples = train_samples[0:n_train]

    nt = int(n_test / 2)
    test_samples_1 = test_samples[0:nt]
    test_samples_2 = test_samples[nt:n_test]
    test_data_1 = test_data[0:nt]
    test_data_2 = test_data[nt:n_test]

    return t_samples, v_samples, test_samples_1, test_samples_2, t_data, v_data, test_data_1, test_data_2
...
def run_cnn(x_t, y_t, x_v, y_v, params, leakage_model, byte, x_test_1, test_data_1, x_test_2, test_data_2, step, fraction):
    ...
    neurons = random.randrange(500, 800, 100)
    conv_layers = random.randrange(1, 1, 1)
...
if __name__ == "__main__":
    ...
    target_params = parameters_ascad_random_key
    ...
    train_samples, val_samples, test_samples_1, test_samples_2, train_data, val_data, test_data_1, test_data_2 = open_datasets(
        "ascad_random_key", 100000, 1000, 10000)

    train_labels = aes_labelize(train_data, l_model, target_byte, target_params)
    validation_labels = aes_labelize(val_data, l_model, target_byte, target_params)
    test_labels_1 = aes_labelize(test_data_1, l_model, target_byte, target_params)
    test_labels_2 = aes_labelize(test_data_2, l_model, target_byte, target_params)

    y_train = to_categorical(train_labels, num_classes=classes)
    y_val = to_categorical(validation_labels, num_classes=classes)
    ...
    kr_nt = int(len(test_samples_1) / (kr_step * kr_fraction))

    # train random MLP
    for model_index in range(number_of_models):
        ge_test_1, ge_test_2, sr_model, kp_krs = run_mlp(train_samples, y_train, val_samples, y_val,
                                                         target_params, l_model, target_byte,
                                                         test_samples_1, test_data_1,
                                                         test_samples_2, test_data_2,
                                                         kr_step, kr_fraction)
    ...
    # train random CNN
    # x_train = train_samples.reshape((train_samples.shape[0], train_samples.shape[1], 1))
    # x_val = val_samples.reshape((val_samples.shape[0], val_samples.shape[1], 1))
    # x_test_1 = test_samples_1.reshape((test_samples_1.shape[0], test_samples_1.shape[1], 1))
    # x_test_2 = test_samples_2.reshape((test_samples_2.shape[0], test_samples_2.shape[1], 1))
    # for model_index in range(number_of_models):
    #     ge_test_1, ge_test_2, sr_model, kp_krs = run_cnn(x_train, y_train, x_val, y_val,
    #                                                      target_params, l_model, target_byte,
